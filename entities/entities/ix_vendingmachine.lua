...
ENT.MaxStock = 4
ENT.Items = {
	{"UNION WATER",		"amnesiacana",	15},
	{"U. PEACH WTR",	"amnesiacanb",	15},
	{"U. SPARKLING",	"amnesiacanc",	15},
	{"GRAPE SODA",		"colabobd",		20},
	{"RB COLA",			"colabob",		35},
	{"RB ENERGY",		"colabobe",		35}
	-- {}
}
...
if (SERVER) then
	...
	function ENT:GetClosestButton(client)
		...
		if (tracePosition) then
			for k, v in ipairs(self.Items) do
				-- cam.Start3D2D(position + forward * 17.51 + right * -19 + up * 5.0, angles, 0.06)
				local position = self:GetPos() + self:GetForward() * 17.53 + self:GetRight() * -24.706 + (self:GetUp() * 4.241 - Vector(0, 0, (k - 1) * 2.06))
	...
	function ENT:Use(client)
		...
		if (!character:IsCombine()) then
			...
			if (self:GetStock(buttonID) > 0) then
				ix.item.Spawn(itemInfo[2], self:GetPos() + self:GetForward() * 19 + self:GetRight() * 4 + self:GetUp() * -30, function(item, entity)
	...
	function ENT:Draw()
		...
		cam.Start3D2D(position + forward * 17.53 + right * -19 + up * 5.0, angles, 0.06)
			...
			for i = 1, 8 do
				...
				if (itemInfo) then
					draw.SimpleText(itemInfo[1], "ixVendingMachine", x, y + halfHeight, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)

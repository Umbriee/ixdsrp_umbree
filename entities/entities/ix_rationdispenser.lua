...
if (SERVER) then
	function ENT:SetRationEnabled(state)
	...
	function ENT:SpawnRation(callback, releaseDelay, rationtype)
	...
	function ENT:StartDispense(rationtype)
		self:SetDisplay(3)
	...
	function ENT:RationUse(rationtype,cid)
		...
		elseif rationtype == "gelsac_cp" then
			timer.Simple(math.random(1.8, 2.2), function()
				self:SetDisplay(8)
				self:EmitSound("ambient/machines/combine_terminal_idle3.wav")
				timer.Simple(10.2, function()
	...
	function ENT:Use(client)
		...
		if client:IsCitizen() then
			...
			if ((cid:GetData("owner_name","UNISSUED") == "UNISSUED") || not cid:GetData("cid")) then
				self:DisplayError(7)
				return
			end
			local ration = "gelsac_s"
			local cidType = (cid:GetData("type", 0) or 0)
			if cidType == 1 then
				ration = "gelsac_prt"
			elseif cidType == 3 then
				ration = "gelsac_l"
			elseif cidType == 4 then
				ration = "gelsac_l"
			end
			self:RationUse(ration,cid)
			self.nextUseTime = CurTime() + 2
		elseif client:IsVortigaunt() then -- Mmm bioitic.
			local cid = client:GetCharacter():GetInventory():HasItem("cid")

...
if SERVER then
	util.AddNetworkString("HelixMat_SetProxyColor")

	function Schema:SetProxyColor(ent, proxyName, vec)
		if not IsValid(ent) then return end
		local data = Schema:GetProxyColorData(ent)
		data[proxyName] = vec
		if Schema.matproxynames[proxyName] then
			net.Start("HelixMat_SetProxyColor")
				net.WriteEntity(ent)
				net.WriteString(proxyName)
				net.WriteVector(vec)
			net.Broadcast()
			return true
		else
	...
	function Schema:SyncProxyColors(ply, ent)
		if not IsValid(ent) or not ent.MatP_ProxyColors then return end

		for proxyName, vec in pairs(ent.MatP_ProxyColors) do
			net.Start("HelixMat_SetProxyColor")
				net.WriteEntity(ent)
				net.WriteString(proxyName)
				net.WriteVector(vec)
			net.Send(ply)
			print("[HelixMatProxying] - Syncing data from "..tostring(ent).." to "..tostring(ply)..".")
		end
	end

	hook.Add("PlayerSpawn", "MatP_SyncProxyColors", function(ply)
		for _, ent in ipairs(ents.GetAll()) do
			Schema:SyncProxyColors(ply, ent)
			--print("[HelixMatProxying] - Prepping data send from "..tostring(ply).." to "..tostring(ent)..".")
		end
	end)
end
...
function Schema:SaveVendingMachines()
	...
	for _, v in ipairs(ents.FindByClass("ix_vendingmachine")) do
		data[#data + 1] = {v:GetPos(), v:GetAngles(), v:GetAllStock(), v:GetSkin()}
	end
